...
def test_create_tables(temp_db_path):
    ...
    with sqlite3.connect(temp_db_path) as conn:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='activity_streams'")
        assert cursor.fetchone() is not None
...
def test_insert_streams(db_manager):
    ...
    streams = [
        {"activity_id": activity_id, "elapsed_seconds": 0, "heart_rate": 140, "speed": 2.5, "timestamp": "2023-01-01T10:00:00Z"},
        {"activity_id": activity_id, "elapsed_seconds": 1, "heart_rate": 142, "speed": 2.6, "timestamp": "2023-01-01T10:00:01Z"},
        {"activity_id": activity_id, "elapsed_seconds": 2, "heart_rate": 145, "speed": 2.7, "timestamp": "2023-01-01T10:00:02Z"},
    ]
...
def test_database_stats(db_manager):
    ...
    # Insert some data
    db_manager.insert_activity({"activity_id": 1, "activity_name": "Run 1", "activity_type": "Run", "activity_date": "2023-01-01"})
    db_manager.insert_activity({"activity_id": 2, "activity_name": "Ride 1", "activity_type": "Ride", "activity_date": "2023-01-02"})
